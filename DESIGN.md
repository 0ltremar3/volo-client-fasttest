...
Reuse the official Beautiful UI-derived Coach prompt bar through its app adapter. The composer keeps a fixed corner radius as it grows, with a warm border and soft shadow. Multiline text aligns with the visible bottom icons; every icon keeps a 44px touch target. Expansion is measured at the compact text width so typing cannot toggle the layout back and forth. Drafts, including dictation, grow from a 44px text row up to the viewport-aware height cap and then scroll internally. Empty, filled, multiline, voice-available, voice-disabled, keyboard submit, and inspiration-filled states are required.

The trailing action shows the orange waveform for an empty draft and Send for a filled draft. The waveform keeps the original single-row composer shell and left inspiration button, replaces only the center textarea with Hold to talk, and places the keyboard return icon on the right. There is no separate footer or expanded voice-input card. Press and hold records; release transcribes and sends the final text. Sliding up changes the orange recording panel to a translucent true-red cancellation state; the instruction uses the same red, and releasing cancels without uploading. Draft text survives mode changes. The independent microphone entry is commented out. While held, the composer itself becomes an orange button containing centered white rounded bars from Ondo UI LiveWaveform in static mode. A soft transparent-to-surface gradient fades the conversation beneath the instruction; cancellation changes only the button to red. Reduced motion shows steady bars. The keyboard glyph is 26px inside a transparent 44px target; empty recognition silently returns to idle. Recognition waiting appears only as a circular spinner in a right-aligned user bubble, with no visible text or composer loader. No interim captions or LiveKit room are opened.

### Move card

Use when the conversation produces a concrete next action. It is not a chat bubble: it combines an 18px action statement, source attribution, check frequency and local time controls, and accept/edit/skip recovery actions. Radius 22px, glass surface, white border, and the same warm elevation family as the Focus card. The pending card owns check-plan setup; Daily never acts as the setup surface for an unscheduled Move. A backend `suggested_schedule` for an explicit daily local time preselects both controls and updates the first-frame summary (for example, `Daily · 12:00`), but remains unpersisted until the user confirms the Move and the follow-up schedule request succeeds.
...
Three real links only: Daily (`/daily`), Coach (`/chat`), and Review (`/review`). Inactive Daily and Review glyphs use tertiary `#9a948e`; the active glyph uses Coach ink. Review uses the exact Figma-exported 32px history glyph rather than a library approximation. The Coach mark sits in a small raised notch and uses the orange ring asset. Navigation is semantic, keyboard accessible, safe-area aware, and at least 93px high including the device inset.
...
With `VITE_MOCK_MODE=false`, Coach, Daily, and Review use the Volo V2 backend. Loading uses stable skeleton surfaces, network failures keep retry actions near the failed work, and completed sessions are read-only. Coach and Echo token deltas stream into the active reply; persisted messages and cards replace optimistic state after reload. Pending Move and Pause cards remain visually distinct from chat bubbles. Period Moves render every active Move from `GET /v2/moves`, already sorted by the backend's next Check Time. Each compact card opens one modal sheet for the operation day's two-state Check, Date, Time, Repeat, Alarm, deletion, and the separate Coach rethink command. Daily no longer reads `period_moves`; Adjustment never changes Check state, and revision confirmation preserves the Schedule. The section collapses from its header and the empty state routes to Coach. No percentage or progress bar is introduced.

## 12. Coach appointments, Pause, and Review

Scheduled Coach cards use nodes `93:8839` and `93:9159`. Appointments are ordered by `scheduled_at`, soonest first; the nearest card sits above two translucent stack layers until expanded. The persistent card is one button and cannot be dismissed from home; tapping it only opens or starts the appointment. Card activation compares `Date.now()` with the absolute `scheduled_at` instant: due and expired appointments start immediately, while future appointments open the full-screen preview where `Start now` permits an early start. The Next Session hero on home opens that same preview. The stack belongs only to Coach home; an open conversation does not repeat it.
...
The explicit `Done` action follows node `96:12022`: for a non-empty conversation it calls the existing end-suggestion path and opens the pending Pause as a full-height bottom sheet over the mounted conversation. An assistant-only conversation is empty; `Done` cancels it and replaces the route with `/daily` without a Pause or Review entry. `Into Your Day` confirms the final fields, completes the session, and replaces the route with `/daily`; `Keep talking` or Escape rejects only the pending Pause and returns focus to the composer. A pending Pause restores the sheet after reload. Backdrop clicks do not discard it.

Review follows nodes `102:14569`, `291:11479`, and `291:11727`. The 350px month calendar uses a 36px orange selected circle and 6px activity dots. Only non-empty Coach, Echo, and Move groups render. Each real history row separates its detail link from a 44px delete icon; deletion requires confirmation and refreshes both the selected day and month activity. Detail uses the confirmed Pause or Echo summary above confirmed Move cards and full read-only messages; its bottom command creates a new conversation.

...
The visible trailing action is stateful: an empty composer shows the supplied 26px `voice.svg` button, while a non-empty composer shows the existing send button. From left to right the idle controls use `inspiration.svg`, `mic.svg`, and `voice.svg`; recording and filled states retain their semantic stop and send symbols. In real mode, the waveform is enabled only for an idle ongoing Coach session without Pause/ending/voice state; it opens the same-route full-screen voice layer. A separate microphone button toggles streaming dictation: while recording, the read-only textarea preserves its original draft and replaces one spoken suffix with a rolling SenseVoice preview about every 1.2 seconds; the second press stops recording, and the SenseVoice final replaces that suffix without being sent. Requesting permission, recording, transcribing, unsupported, empty, and failed states stay within the composer. A caption-stream failure falls back to stop-then-transcribe, and a final failure restores the original draft. The voice layer preserves the Coach atmosphere, uses one centered audio signal, state copy, a scrollable transcript, microphone/hangup controls, a quiet corner input-device icon, 44px targets, safe areas, and reduced motion. It never exposes camera or screen controls.
